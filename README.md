...
The `GVI` R package helps researchers to calculate the Green Visibility
...
3.  Install GVI:
    ...
    -   Install `GVIGVI` is still in very active development. Therefore,
        the package is also not on CRAN yet. You can install the latest
...
Digital Surface Model map. A buffer is applied on the observer position,
and for every point on the perimeter a Line of Sight (LoS) is being
calculated. Visibility along every point on the LoS is computed using a
C++ implementation of Bresenham’s line algorithm \[[Bresenham
...
functions. The input for the observer location is an
...
single sf point object containing the VGVI of every point. This function
supports parallel computation on Windows, Linux and MacOS, though it is
highly recommended to use Linux or MacOS.

# Examples

For the example we will use a [Digital Elevation Model
...
uploaded to in a separate GitHub repository and needs to be downloaded
first.
...
`vgvi_from_sf` function. This feature represents roads, that can be
...
Compute the VGVI along the line feature. Since the resolution is 5m,
points along the line feature will be generated every 5m.
