#' @title Line of Sight
#' @description Computes a single Line of Sight (LoS) using a Digital Surface Model (DSM) matrix.
#'
...
LoS <- function(rc1, r0, c0, dsm_mat, observerHeight) {
  ...
  pointsZ <- GVI::bresenham(x1 = c0, y1 = r0, x2 = c1, y2 = r1)
  ...
  tangents <- GVI::tangents(x1 = c0, y1 = r0, height0 = observerHeight, xy2 = pointsZ, dsm_profile = dsm_profile)
  visibile <- GVI::tangents(tangents)
