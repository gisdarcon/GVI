...
vgvi <- function(viewshed, sf_start, greenspace, m = 0.5, b = 8, mode = c("logit", "exponential")) {
  # Get XY coordinates of visible cells
  xy <- viewshed %>% 
    terra::xyFromCell(which(viewshed[] == 1))
  
  # Calculate distance
  dxy = round(sqrt((sf::st_coordinates(sf_start)[1] - xy[,1])^2 + (sf::st_coordinates(sf_start)[2] - xy[,2])^2))
